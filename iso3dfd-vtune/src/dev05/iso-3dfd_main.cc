...
void initialize(float* ptr_prev, float* ptr_next, float* ptr_vel, Parameters* p, size_t nbytes, size_t n1_Tblock, size_t n2_Tblock, size_t n3_Tblock){
			...
			for(int bx=HALF_LENGTH; bx<n1End; bx+=n1_Tblock){	
				...
				for(int iz=bz; iz<izEnd; iz++) {
					for(int iy=by; iy<iyEnd; iy++) {
						...
						ptr_prev[iz*p->n2*p->n1 + iy*p->n1 + ix] = 0.0f;
                                		ptr_next[iz*p->n2*p->n1 + iy*p->n1 + ix] = 0.0f;
                                		ptr_vel[iz*p->n2*p->n1 + iy*p->n1 + ix] = 2250000.0f*DT*DT;//Integration of the v² and dt² here
...
#  error "HALF_LENGTH not implemented"
	...
	initialize(p.prev, p.next, p.vel, &p, nbytes, p.n1_Tblock, p.n2_Tblock, p.n3_Tblock);
